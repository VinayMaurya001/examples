package com.examples.programs;

public class CallByValueOrReference {

	public static void main(String[] args) {

		callbyValueExample();
		 callByReferenceExample();
	}

	private static void callbyValueExample() {
		System.out.println("Call by Value:");
		int a = 30;
		int b = 45;
		System.out.println("Before swapping, a = " + a + " and b = " + b);
		swapFunctionUsingCallByValue(a, b);
		System.out.println("Now, Before and After swapping values will be same here:");
		System.out.println("After swapping, a = " + a + " and b is " + b+"\n");
	}

	public static void swapFunctionUsingCallByValue(int a, int b) {
		System.out.println("Before swapping(Inside), a = " + a + " b = " + b);
		int c = a;
		a = b;
		b = c;
		System.out.println("After swapping(Inside), a = " + a + " b = " + b);
	}

	private static void callByReferenceExample() {
		System.out.println("Call by reference:");
		IntWrapper a = new IntWrapper(30);
		IntWrapper b = new IntWrapper(45);
		System.out.println("Before swapping, a = " + a.a + " and b = " + b.a);
		swapFunctionUsingCallByReference(a, b);
		System.out.println("Now, Before and After swapping values will be different here:");
		System.out.println("After swapping, a = " + a.a + " and b is " + b.a+"\n");
	}

	public static void swapFunctionUsingCallByReference(IntWrapper a, IntWrapper b) {
		System.out.println("Before swapping(Inside), a = " + a.a + " b = " + b.a);
		// Swap n1 with n2
		IntWrapper c = new IntWrapper(a.a);
		a.a = b.a;
		b.a = c.a;
		System.out.println("After swapping(Inside), a = " + a.a + " b = " + b.a);

	}

}

class IntWrapper {
	public int a;

	public IntWrapper(int a) {
		this.a = a;
	}
}
